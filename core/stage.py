...
from normal_stage_performance_model import NormalStagePrediction
from first_stage_performance_model import FirstStagePrediction
from first_stage_no_aggregate_performance_model import NoAggregatePrediction
import numpy as np
import warnings
...
class FirstStage(Stage):
    ...
    def compute_inpute_size(self):
        ...
        if self.fileScan_fields != 0 and self.fileScan_table != "":
            if self.fileScan_table in self.table_sizes_500G:
                ...
                self.input_data_size = round(self.input_data_size, 1)
    ...
    def predict_runtime_output_size(self, resource_config):
        ...
        test_x_datasize = np.array([[
                                    ...
                                    self.broadcast_join_depth, self.project_depth,0]])
        ...
        test_x_runtime = np.array([[resource_config["executor_cores"], resource_config["executor_mem"],
                            ...
                            self.broadcast_joins, self.projects, self.hash_aggregate, self.exchange_partition,
                            self.broadcast_join_depth, self.project_depth, 0,480]])
        ...
        print("First Stage prediction: {}, estimated_input_size {} MiB, estimated_output_size {} MiB, estimated runtime {} seconds".format(self.to_model_str(), self.input_data_size, self.estimate_output_size, self.estimate_runtime))
...
class NormalStage(Stage):
    ...
    def predict_runtime_output_size(self,resource_config):
        ...
        test_X_runtime = np.array([[resource_config["executor_cores"], resource_config["executor_mem"],
                            ...
                            self.aggregate_depth, self.sort_depth, self.sort_merge_depth, self.filter_depth,
                            self.union_depth, 200]])
        self.estimate_output_size, self.estimate_runtime = normal_predictor.predict_stage(test_X_datasize,test_X_runtime)
        self.estimate_runtime = round(self.estimate_runtime, 1)

        print("Normal Stage prediction: {}, estimated_input_size {} MB, estimated_output_size {} MiB, estimated runtime {} seconds".format(
              self.to_node_str(), self.input_data_size, self.estimate_output_size, self.estimate_runtime))
        return self.estimate_output_size, self.estimate_runtime
